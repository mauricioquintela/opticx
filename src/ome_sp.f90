...
contains
   ...
   complex*16 amu,amup,aux1,aux2,aux3,aux4,aux5,aux6,factor
   complex*16 vme_der
   complex*16 berry_eigen1,berry_eigen2,berry_eigen
   complex*16 hk_ev_neigh,vme_neigh
 
   ! === dimensionality flags: computed ONCE, never inside any loop ===
   ...
   ! --- Neighbour kernel calls: one branch per direction, no repeated checks ---
   if (active_x) then
      rkx_neigh=rkx-dk
      rky_neigh=rky
      rkz_neigh=rkz
   ...
   ! 7th index will be the central one, independently of directions defined
   rkx_neigh=rkx
   rky_neigh=rky
   rkz_neigh=rkz
   call get_vme_kernels_ome(rkx_neigh,rky_neigh,rkz_neigh,norb,skernel, &
      ...
      do nnp=1,norb
         do ialpha=1,norb
            do ialphap=1,norb
               ...
               ! berry_eigen1(1,...) stays 0 when inactive (initialised above)
               ...
               ! berry_eigen1(2,...) stays 0 when inactive (initialised above)
               ...
               ! berry_eigen1(3,...) stays 0 when inactive (initialised above)
         ...
         do nj=1,3
            !if BC's is diverging at degenerate points, set it to zero
            ...
            aux5=vme_neigh(5,nj,nn,nnp)
            aux6=vme_neigh(6,nj,nn,nnp)
 
            ! For inactive directions the differences are zero.
            if (active_x) then
               vme_der(1,nj,nn,nnp)=(aux3-aux1)/(2.0d0*dk)
            else
               vme_der(1,nj,nn,nnp)=0.0d0
            end if
            if (active_y) then
               vme_der(2,nj,nn,nnp)=(aux4-aux2)/(2.0d0*dk)
            else
               vme_der(2,nj,nn,nnp)=0.0d0
            end if
            if (active_z) then
               vme_der(3,nj,nn,nnp)=(aux6-aux5)/(2.0d0*dk)
            else
               vme_der(3,nj,nn,nnp)=0.0d0
            end if
            ...
            if (active_x) then
               ...
               vme_der_phase(1,nj,nn,nnp)=0.0d0
            end if
            if (active_y) then
               vme_der_phase(2,nj,nn,nnp)=(ph4-ph2)/(2.0d0*dk)
            else
               vme_der_phase(2,nj,nn,nnp)=0.0d0
            end if
            if (active_z) then
               vme_der_phase(3,nj,nn,nnp)=(ph6-ph5)/(2.0d0*dk)
            else
               vme_der_phase(3,nj,nn,nnp)=0.0d0
            end if
